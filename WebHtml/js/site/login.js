        ...
        if (document.cookie.username != '') {
            ...
            for (var i = 0; i < array.length; i++) {
                if (array[i].indexOf('username') != -1 && array[i].length > 12) {
                    $("input[name=username]").val(array[i].slice(9));
...
function checkUserName() {
    var value = $("input[name=userName]").val(), password = $("input[name=password]").val();
    if (phoneReg.test(value) || emailReg.test(value) || value != null) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.State == 1) {
                ...
                $.cookie("token", window.token);
                window.location.href = "/account/project_home.html"
                initProjectVeiw();
            } else {
